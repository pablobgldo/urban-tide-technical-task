...
Before running the tests, set the PYTHONPATH environment variable to include the 'src' directory. This way, Python can locate all necessary modules. In the root directory of the project, run the following command:```bash export PYTHONPATH=./src```  
2. Run the tests:
The tests are written using Pytest. To run all tests, use the following command: ```bash python -m pytest``` This will execute all test files located in the project's tests folder.
